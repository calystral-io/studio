...
curl -H 'Authorization: Bearer mock-reader-token' \
  'localhost:8080/api/v1/anchors?page_size=25&type=Employee'
```

The anchors browser serves ~142 seeded bitemporal anchors (Employee, Department,
...
`UNIMPLEMENTED` gap to a 501 `/errors/upstream/unimplemented` (surface=anchors).
...
Sync note: only `query.proto` is exercised today (the anchors read path);
